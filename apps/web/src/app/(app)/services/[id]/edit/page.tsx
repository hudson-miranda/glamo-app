/**
 * GLAMO - Service Edit Page
 * Enterprise service editing with unsaved changes detection
 * 
 * @version 1.0.0
 * @description Full-featured service editing form
 */
...
import { InlineCreateModal } from '@/components/common/InlineCreateModal';
import { ConfirmDialog } from '@/components/common/ConfirmDialog';
import { useService } from '@/hooks/useServices';
import { useToast } from '@/hooks/useToast';
import { cn, formatCurrency } from '@/lib/utils';
import type { BusinessSegment, ServiceFormData, Service, Professional } from '@/types';
...
function DurationPicker({ value, onChange, error }: DurationPickerProps) {
  ...
  useEffect(() => {
    setIsCustom(!DURATION_PRESETS.includes(value));
...
function PriceInput({ value, onChange, promoPrice, onPromoPriceChange, error }: PriceInputProps) {
  const [showPromo, setShowPromo] = useState(promoPrice !== undefined && promoPrice > 0);
...
export default function EditServicePage() {
  ...
  const { service, isLoading, error, updateService, deleteService, isUpdating, isDeleting, refetch } = useService(serviceId);
  ...
  const [initialData, setInitialData] = useState<Partial<ServiceFormData>>({});
  ...
  useEffect(() => {
    if (service) {
      ...
      setFormData(data);
      setInitialData(data);
      setSelectedProfessionals(service.professionals?.map((p: any) => p.id) || []);
    }
  }, [service]);
  ...
  useEffect(() => {
    const hasChanges = JSON.stringify(formData) !== JSON.stringify(initialData) ||
      JSON.stringify(selectedProfessionals.sort()) !== JSON.stringify((service?.professionals?.map((p: any) => p.id) || []).sort());
    setIsDirty(hasChanges);
  }, [formData, selectedProfessionals, initialData, service]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Erro',
        description: error instanceof Error ? error.message : 'Falha ao atualizar serviço',
        variant: 'destructive',
      });
    }
  };
        ...
        <Card className="py-16">
          <CardContent className="flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">
              O serviço que você está tentando editar não existe ou foi removido.
            </p>
            <Link href="/services">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Permitir agendamento online deste serviço
                        </p>
                      </div>
                    </div>
                    <Switch
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-800">
          <Button
            type="button"
            variant="outline"
            onClick={() => handleNavigation(`/services/${serviceId}`)}
            disabled={isUpdating}
          >
            Cancelar
          </Button>
